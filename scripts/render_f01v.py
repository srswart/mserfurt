"""Render f01v — verso, leaf 1.  Modest private manuscript layout."""
...
X_HEIGHT_MM     = 3.0
LINE_HEIGHT_MM  = 11.0
BASELINE_IN_CANVAS_MM = 7.5
LINE_SPACING_MM = 8.0
LINES_PER_PAGE  = int(TEXT_H_MM / LINE_SPACING_MM)   # 22

# Verso: gutter on the right → outer margin is left margin
MARGIN_LEFT_MM  = MARGIN_OUTER_MM

FOLIO_JSON  = ROOT / "output-live" / "f01v.json"
RENDER_OUT  = ROOT / "render-output" / "f01v.png"
DEBUG_DIR   = ROOT / "debug" / "f01v_page"
...
def save_progress(label):
    ...
    img.save(str(DEBUG_DIR / f"f01v_{label}.png"), dpi=(DPI, DPI))
    img.save(str(DESKTOP / "f01v_page.png"),        dpi=(DPI, DPI))
